/* Clean Functional V1 Foundation shell — Slice 4 Search & Intent. */
(function foundationShell() {
  ...
  const routeRegistry = new Set(['discover', 'mine', 'preview', 'search/q1', 'search/q2', 'search/q3', 'search/results', 'search/partial', 'search/no-results', 'initiative-detail', 'place-detail', 'offering-detail', 'job']);
  ...
  const makeDraft = (type, value = {}) => {
    ...
    draft.createdFrom = value.createdFrom === 'search' ? 'search' : null;
  ...
  const normalizeFlow = value => {
    ...
    return { ...defaults, type, step: typeof value.step === 'string' ? value.step : null, draft: normalizeDraft(value.draft, createTypes.has(type) ? type : null), targetId: typeof value.targetId === 'string' ? value.targetId : null, targetType: typeof value.targetType === 'string' ? value.targetType : null, validationErrors: Array.isArray(value.validationErrors) ? value.validationErrors.filter(item => typeof item === 'string').slice(0, 5) : [] };
  };
  ...
  const resolveRoute = (route, createdItems = appState.createdItems) => {
    if (route === 'discover' || route === 'mine' || route === 'preview' || searchRoutes.has(route)) return route;
  ...
  const restoreRouteAndContext = () => {
    ...
    if (ui.overlay?.type === 'replace-draft-confirm' && !resultRoutes.has(route)) ui = { overlay: null };
  ...
  const renderCreateForm = state => {
    ...
    const backRoute = draft.createdFrom === 'search' ? `search/${evaluateSearch(draft.searchAnswers).outcome}` : 'discover';
    const provenance = draft.createdFrom === 'search' ? '<p class="privacy-note" data-testid="search-provenance">טיוטה שנוצרה מהחיפוש. היא ניתנת לעריכה ולא תפורסם בלי אישור מפורש.</p>' : '';
  ...
  const renderMine = state => {
    ...
    const cards = items.length ? items.map(item => `<article class="card created-item" data-created-id="${escapeHtml(item.id)}"><span class="context">${draftTypeLabel(item.type)} · ${escapeHtml(item.area)}</span><h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.description)}</p><span class="signals"><span>פורסם בחשבון שלך</span>${item.createdFrom === 'search' ? '<span>נוצר מחיפוש</span>' : ''}</span></article>`).join('') : '<p class="lede">עדיין אין כאן פרסומים.</p>';
  ...
  const beginSearchDraft = () => {
    ...
    transition({ flow: { type: 'post', step: 'form', draft, validationErrors: [] }, ui: { overlay: null } });
  ...
  const requestSearchDraft = () => {
    ...
    if (existing) { transition({ ui: { overlay: { type: 'replace-draft-confirm' } } }); render(appState); return; }
    beginSearchDraft();
  };
  ...
  const dispatch = (action, payload = {}, context = {}) => {
    switch (action) {
      ...
      case 'confirm-publish-search-need': beginSearchDraft(); break;
  ...
  const handleAction = event => { const target = event.target.closest('[data-action], [data-route]'); if (!target) return; const action = target.dataset.action || 'navigate'; event.preventDefault(); dispatch(action, { route: target.dataset.route || 'discover', entityType: target.dataset.entityType, entityId: target.dataset.entityId, createType: target.dataset.createType, searchField: target.dataset.searchField, searchValue: target.dataset.searchValue }, appState.context); };
