...
(function foundationShell() {
  ...
  const initialState = () => ({ route: { name: 'discover' }, context: { entityType: null, entityId: null }, ui: { overlay: null }, auth: { status: 'anonymous' }, flow: { type: null, step: null, draft: null, targetId: null, targetType: null, validationErrors: [] }, search: { answers: { need: '', area: '', context: '' }, outcome: null }, savedIntents: [], createdItems: [], connections: [], pendingAction: null });
  ...
  const normalizeConnection = value => {
    ...
    return { id: `${value.actionType}:${value.entityId}`, actionType: value.actionType, entityType: value.entityType, entityId: value.entityId, status: value.status === 'handled' ? 'handled' : 'requested', createdAt: typeof value.createdAt === 'number' ? value.createdAt : 0 };
  };
  ...
  const transition = patch => {
    appState = { ...appState, ...patch, route: patch.route ? { ...appState.route, ...patch.route } : appState.route, context: patch.context ? { ...appState.context, ...patch.context } : appState.context, ui: patch.ui ? { ...appState.ui, ...patch.ui } : appState.ui, auth: patch.auth ? { ...appState.auth, ...patch.auth } : appState.auth, flow: patch.flow ? { ...appState.flow, ...patch.flow } : appState.flow, search: patch.search ? { ...appState.search, ...patch.search } : appState.search, savedIntents: patch.savedIntents || appState.savedIntents, createdItems: patch.createdItems || appState.createdItems, connections: patch.connections || appState.connections };
  ...
  const restoreRouteAndContext = () => {
    ...
    const context = parsed && ['initiative', 'place', 'offering', 'job', 'inspiration', 'created', 'managed-initiative', 'saved-intent'].includes(parsed.entityType) ? { entityType: parsed.entityType, entityId: parsed.entityId } : { entityType: null, entityId: null };
    transition({ route: { name: route }, context, ui, auth: persisted.auth, flow, search, savedIntents: persisted.savedIntents, connections: persisted.connections, pendingAction, createdItems: persisted.createdItems });
  ...
  const renderDetail = state => { const item = seedByTypeAndId(state.context.entityType, state.context.entityId); if (!item) return renderDiscover(state); const request = state.connections.find(connection => connection.entityId === item.id && connection.entityType === item.type); const protectedAction = item.type === 'initiative' ? actionButton('להצטרף ליוזמה', 'join', ` data-entity-type="${item.type}" data-entity-id="${item.id}"`) : item.type === 'offering' ? actionButton('להתחבר', 'connect', ` data-entity-type="${item.type}" data-entity-id="${item.id}"`) : ''; const confirmation = request ? `<div class="confirmation" role="status" data-testid="connection-status"><p class="eyebrow">${request.status === 'handled' ? 'טופל' : 'הבקשה נשלחה'}</p><h2>${request.actionType === 'join' ? 'הצטרפת ליוזמה — הבקשה נשמרה' : 'בקשת החיבור נשלחה'}</h2><p>אפשר לראות את המצב גם ב״שלי״. פעולה חוזרת לא תיצור בקשה נוספת.</p><button class="secondary" data-action="navigate" data-route="mine">לשלי</button></div>` : protectedAction; return `<section class="intro detail-view"><button class="text-link" data-action="navigate" data-route="discover">← חזרה לגילוי</button>${heading(item.eyebrow, item.title, item.description)}<div class="detail-meta"><p>${item.area}</p><span>מתאים להורים</span></div>${confirmation}</section>`; };
