...
(function foundationShell() {
  ...
  const renderSearchOutcome = state => {
    ...
    return `<section class="intro search-view search-outcome" data-testid="search-${evaluated.outcome}"><button class="text-link" data-action="navigate" data-route="search/q3">← לעריכת התשובה האחרונה</button>${content}${noPlace}${savedNotice}${cards ? `<div class="search-results">${cards}</div>` : ''}<div class="search-next-actions"><button class="secondary" data-action="edit-search">לשנות או להרחיב חיפוש</button>${isSaved ? '' : actionButton('לשמור כוונה באופן פרטי', 'request-save-intent')}${inspirationAction}<button class="secondary" data-action="publish-search-need">לפרסם מה אני צריכה</button>${notNow}</div></section>`;
  };

  const field = (label, name, value, multiline = false, required = false) => `<label class="field"><span>${label}${required ? ' *' : ''}</span>${multiline ? `<textarea name="${name}" data-field="${name}" rows="4">${escapeHtml(value)}</textarea>` : `<input name="${name}" data-field="${name}" value="${escapeHtml(value)}" autocomplete="off">`}${name === 'description' ? '<small class="field-hint">לא לכתוב שמות ילדים או פרטים אישיים שמאפשרים לזהות אותם.</small>' : ''}</label>`;
  ...
  const renderCreateForm = state => {
    ...
    const optional = draft.type === 'initiative' ? `${field('גילאי הילדים', 'childAges', draft.childAges)}${field('ימים ושעות', 'daysTimes', draft.daysTimes)}${field('מקום', 'place', draft.place)}${field('צוות / מדריכה', 'educator', draft.educator)}${field('פעילות', 'activity', draft.activity)}${field('תקציב', 'budget', draft.budget)}${field('כמה אנשים צריך', 'peopleNeeded', draft.peopleNeeded)}` : draft.type === 'offering' ? `${field('סוג ההצעה', 'serviceType', draft.serviceType)}${field('מחיר', 'price', draft.price)}` : draft.type === 'place' ? `${field('סוג המקום / שימוש', 'place', draft.place)}${field('זמינות', 'availability', draft.availability)}${field('קיבולת', 'capacity', draft.capacity)}${field('מתאים לילדים', 'childSuitability', draft.childSuitability)}${field('מתקנים', 'facilities', draft.facilities)}${field('מחיר', 'price', draft.price)}${field('כתובת מדויקת (פרטית, לא תפורסם)', 'privateAddress', draft.privateAddress)}${field('תמונות', 'photos', draft.photos)}` : '';
  ...
  const renderSuccess = state => { const item = createdById(state.context.entityId || state.flow.targetId); if (!item) return renderDiscover(state); return `<section class="intro creation-view publish-success" data-testid="publish-success">${heading('הפרסום נשמר', 'הפרסום פורסם', 'הוא נשמר בחשבון ההדגמה שלך, ואפשר לחזור אליו ב״שלי״.')}<div class="preview-card"><p class="eyebrow">${draftTypeLabel(item.type)} · ${escapeHtml(item.area)}</p><h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.description)}</p>${publicDraftDetails(item)}</div><p class="privacy-note">המידע שמופיע כאן ובפרסום הוא ציבורי. כתובת מדויקת ופרטי קשר נשארים פרטיים.</p><div class="form-actions"><button class="primary" data-action="open-created" data-item-id="${item.id}">לצפות בפרסום <span>←</span></button><button class="secondary" data-action="navigate" data-route="mine">לראות ב״שלי״</button><button class="secondary" data-action="begin-search">להמשיך לגלות</button></div></section>`; };
  ...
  const renderMine = state => {
    ...
    return `<section class="intro mine-view" data-testid="mine"><button class="text-link" data-action="navigate" data-route="discover">← חזרה לגילוי</button>${heading('שלי', 'הדברים שלי', 'כוונות פרטיות, פרסומים וחיבורים נשמרים כאן בנפרד.')}<h2 class="section-title">מה שאני מחפשת</h2><p class="privacy-note">הכוונות האלה פרטיות ואינן פוסט ציבורי.</p><div class="created-list">${intents}</div><h2 class="section-title">היוזמות שלי</h2><div class="created-list">${initiatives.length ? initiatives.map(item => itemCard(item, true)).join('') : emptyMy('עדיין אין יוזמה שפרסמת.', 'start', 'ליצור יוזמה')}</div><h2 class="section-title">מה שפרסמתי</h2><div class="created-list">${published.length ? published.map(item => itemCard(item)).join('') : emptyMy('עדיין אין פרסומים נוספים.', 'start', 'ליצור פרסום')}</div><h2 class="section-title">החיבורים שלי</h2><div class="created-list">${connections}</div></section>`;
  };
  ...
  const renderManageInitiative = state => { const item = createdById(state.context.entityId); if (!item || item.type !== 'initiative') return renderMine(state); const needs = [['people', 'אנשים', item.peopleNeeded], ['place', 'מקום', item.place], ['educator', 'אשת חינוך / מקצוע', item.educator]].filter(([, , value]) => value); return `<section class="intro detail-view" data-testid="manage-initiative"><button class="text-link" data-action="navigate" data-route="mine">← חזרה לשלי</button>${heading('ניהול יוזמה', escapeHtml(item.title), 'הצרכים המקבילים נשארים באותה יוזמה; אפשר לעצור או לחדש כל אחד בנפרד.')}<div class="preview-card"><p><strong>סטטוס:</strong> פורסמה</p><p><strong>אזור:</strong> ${escapeHtml(item.area)}</p>${needs.length ? needs.map(([key, label, value]) => { const paused = item.needStates?.[key] === 'paused'; return `<div class="manage-need" data-need-key="${key}"><p><strong>${label}:</strong> ${escapeHtml(value)} · ${paused ? 'מושהה' : 'פעיל'}</p><button class="secondary" data-action="set-initiative-need-state" data-item-id="${item.id}" data-need-key="${key}" data-need-state="${paused ? 'open' : 'paused'}">${paused ? 'לחדש צורך' : 'להשהות צורך'}</button></div>`; }).join('') : '<p>לא הוגדר צורך נוסף בפרסום הזה.</p>'}</div><p class="privacy-note">אין כאן ניהול משתתפים, תשלומים או פרטי ילדים.</p><button class="secondary" data-action="open-created" data-item-id="${item.id}">לצפות בפרסום</button></section>`; };
  const renderSavedIntent = state => { const intent = state.savedIntents.find(item => item.id === state.context.entityId); if (!intent) return renderMine(state); return `<section class="intro detail-view" data-testid="saved-intent-detail"><button class="text-link" data-action="navigate" data-route="mine">← חזרה לשלי</button>${heading('כוונה פרטית', needLabels[intent.answers.need], `${intent.answers.area} · ${contextLabels[intent.answers.context]}`)}<p class="privacy-note">גלוי רק לך. הכוונה לא פורסמה ולא הופכת לפוסט.</p><button class="primary" data-action="resume-saved-intent" data-intent-id="${intent.id}">לחזור לחיפוש הזה <span>←</span></button></section>`; };

  const renderOverlay = overlay => {
    if (!overlay) return '';
    if (overlay.type === 'account-gate') return `<div class="scrim open" data-action="cancel-gate"></div><section class="sheet open account-gate" role="dialog" aria-modal="true"><button class="close" data-action="cancel-gate">×</button>${heading('נדרש חשבון', 'מתחברות כדי לשמור את הפעולה', 'החשבון שומר את הבקשה או הפרסום שלך ומחזיר אותך בדיוק לפעולה שבחרת. זהו חשבון הדגמה — ללא פרטים אישיים.')}<div class="sheet-options"><button data-action="demo-auth">להתחבר ולהמשיך</button><button data-action="cancel-gate">ביטול</button></div></section>`;
