...
(function foundationShell() {
  ...
  const renderCreateForm = state => {
    ...
    const optional = draft.type === 'initiative' ? `${choiceField('גילאי הילדים', 'childAges', draft.childAges, ['פעוטות', 'ילדים בגיל גן', 'ילדים גדולים יותר', 'טווח גילאים רחב', 'עוד לא יודעת'])}${reveal(Boolean(draft.childAges), choiceField('מתי זה יכול לקרות?', 'daysTimes', draft.daysTimes, ['בקרים', 'צהריים', 'אחר הצהריים', 'ערב', 'סופי שבוע', 'גמישה']))}${reveal(Boolean(draft.daysTimes), choiceField('יש כבר מקום?', 'place', draft.place, ['יש כבר מקום', 'עדיין אין מקום', 'פתוחה להצעות', 'עוד לא יודעת']))}${reveal(Boolean(draft.place), choiceField('צריך אשת מקצוע?', 'educator', draft.educator, ['כן', 'לא', 'אולי', 'עוד לא יודעת']))}${reveal(Boolean(draft.educator), choiceField('כמה משפחות?', 'peopleNeeded', draft.peopleNeeded, ['עוד 1', 'עוד 2–3', 'עוד 4–5', 'יותר', 'עוד לא יודעת']))}${reveal(Boolean(draft.peopleNeeded), `${field('פעילות', 'activity', draft.activity)}${field('תקציב', 'budget', draft.budget)}`)}` : draft.type === 'offering' ? `${choiceField('סוג ההצעה', 'serviceType', draft.serviceType, ['שירות מקצועי', 'פעילות', 'ידע / עזרה', 'מקום', 'שיתוף פעולה'])}${reveal(Boolean(draft.serviceType), choiceField('מתי זה יכול לקרות?', 'availability', draft.availability, ['בקרים', 'צהריים', 'ערב', 'גמישה', 'בתיאום']))}${reveal(Boolean(draft.availability), field('מחיר', 'price', draft.price))}` : draft.type === 'place' ? `${choiceField('מתי אפשר להשתמש במקום?', 'availability', draft.availability, ['בקרים', 'צהריים', 'אחר הצהריים', 'ערב', 'סופי שבוע', 'גמיש / בתיאום'])}${reveal(Boolean(draft.availability), choiceField('לכמה משפחות המקום מתאים?', 'capacity', draft.capacity, ['עד 3 משפחות', '4–6 משפחות', '7–10', 'יותר', 'עוד לא יודעת']))}${reveal(Boolean(draft.capacity), choiceField('למי המקום מתאים?', 'childSuitability', draft.childSuitability, ['מתאים לפעוטות', 'מתאים לילדים גדולים יותר', 'מתאים לטווח גילאים רחב', 'צריך לבדוק יחד']))}${reveal(Boolean(draft.childSuitability), `${field('מתקנים', 'facilities', draft.facilities)}${field('מחיר', 'price', draft.price)}${field('כתובת מדויקת (פרטית, לא תפורסם)', 'privateAddress', draft.privateAddress)}${field('תמונות', 'photos', draft.photos)}`)}` : '';
