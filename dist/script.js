...
(function foundationShell() {
  ...
  const renderCard = kind => { const item = seed[kind]; if (isHiddenEntity(item.type, item.id)) return ''; const facts = [['אזור', item.area]]; return `<article class="card card-${kind} ${kind === 'initiative' ? 'featured' : ''}" data-entity-id="${item.id}"><div class="card-kicker"><span class="context">${item.eyebrow}</span><span class="card-index">${kind === 'initiative' ? '01' : kind === 'place' ? '02' : kind === 'offer' ? '03' : '04'}</span></div><div class="card-copy"><h2>${item.title}</h2><p>${item.description}</p></div><dl class="card-facts">${facts.map(([label, value]) => `<div><dt>${label}</dt><dd>${value}</dd></div>`).join('')}</dl><div class="card-action">${actionButton('לפרטים', 'open-detail', ` data-entity-type="${item.type}" data-entity-id="${item.id}"`)}</div></article>`; };
  const renderDiscover = () => `<section class="intro discover-opening"><div class="opening-copy">${heading('קהילה שמפנה מקום לשני הצדדים', 'גם לעבוד.<br><em>גם להיות קרובים.</em>', 'להמשיך לעבוד, ללמוד ולהתפתח — ולהישאר קרובים לילדים.')}</div><div class="opening-action">${actionButton('מה את מחפשת עכשיו?', 'open-start')}</div></section><nav class="filters"><button class="filter active" data-action="navigate" data-route="discover">הכול</button><button class="filter" data-action="navigate" data-route="inspiration">השראה</button><button class="filter" data-action="navigate" data-route="discover">עבודה והתפתחות</button></nav><section class="feed"><header class="feed-intro"><span class="context">מה קורה עכשיו</span><h2>דברים שאפשר להתחיל מהם</h2></header><div class="feature-slot">${renderCard('initiative')}</div><div class="discovery-pair"><div class="place">${renderCard('place')}</div><div class="care">${renderCard('offer')}</div></div><div class="open-item">${renderCard('job')}</div><article class="inspiration-card"><div>${heading('בואי תראי מה כבר קיים', 'רעיונות שאפשר להתחיל מהם', 'דוגמאות קטנות למה שגם וגם יכול להיראות — ואז עורכות את זה כך שיתאים לך.')}</div><button class="secondary" data-action="navigate" data-route="inspiration">לכל ההשראות</button></article><article class="action-card">${heading('לא מצאת עדיין?', 'ספרי מה יעזור לך — ונחפש.')}${actionButton('להתחיל לחפש', 'begin-search')}</article></section>`;
  ...
  const renderDetail = state => { const item = seedByTypeAndId(state.context.entityType, state.context.entityId); if (!item) return renderDiscover(state); if (isHiddenEntity(item.type, item.id, state)) return renderUnavailable(); const request = state.connections.find(connection => connection.entityId === item.id && connection.entityType === item.type && connection.status !== 'inactive'); const protectedAction = item.type === 'initiative' ? actionButton('לשלוח בקשת הצטרפות', 'join', ` data-entity-type="${item.type}" data-entity-id="${item.id}"`) : item.type === 'offering' ? actionButton('לשלוח בקשת חיבור', 'connect', ` data-entity-type="${item.type}" data-entity-id="${item.id}"`) : ''; const confirmation = request ? `<div class="confirmation" role="status" data-testid="connection-status"><p class="eyebrow">${request.status === 'handled' ? 'טופל' : 'הבקשה נשלחה'}</p><h2>${request.actionType === 'join' ? 'הבקשה נשלחה' : 'בקשת החיבור נשלחה'}</h2><p>נעדכן אותך כשיהיה שינוי. אפשר לעקוב אחריה באזור האישי.</p><button class="secondary" data-action="navigate" data-route="mine">לשלי</button></div>` : ''; const jobBoundary = item.type === 'job' ? '<div class="privacy-note" data-testid="job-outbound-boundary"><strong>מקור:</strong> דוגמת תוכן בלבד. זו אינה משרה חיה או מאומתת, ולכן אין קישור להגשה. הגשות למשרות חיות מתבצעות מחוץ לגם וגם.</div>' : ''; return `<section class="intro detail-view detail-${item.type}"><button class="text-link" data-action="navigate" data-route="discover">← חזרה לגילוי</button><header class="detail-identity">${heading(item.eyebrow, item.title, item.description)}<div class="detail-meta"><p>${item.area}</p></div></header><section class="detail-practical">${jobBoundary}${confirmation}</section><footer class="detail-actions">${protectedAction}${safetyActions(item)}</footer></section>`; };
  const renderInspiration = state => {
    ...
    const cards = Object.values(inspirationModels).map((item, index) => `<article class="inspiration-card inspiration-${index === 0 ? 'feature' : 'note'}" data-inspiration-card-id="${item.id}"><span class="inspiration-number">0${index + 1}</span><div><span class="context">יש עוד אמא אחת?</span><h2>${item.title}</h2><p>${item.summary}</p></div><button class="secondary" data-action="open-inspiration" data-inspiration-model-id="${item.id}">לפרטים</button></article>`).join('');
    return `<section class="intro inspiration-view" data-testid="inspiration-library"><button class="text-link" data-action="navigate" data-route="discover">← חזרה לגילוי</button><div class="inspiration-opening">${heading('רעיונות שאפשר להתחיל מהם', 'רעיונות שאפשר להתחיל מהם', 'אלה רק נקודות פתיחה. אפשר לבחור רעיון, להתאים אותו למה שיש לך — ולפרסם רק אם תרצי.')}</div><div class="created-list inspiration-list">${cards}</div></section>`;
  };
  ...
  const renderMine = state => {
    ...
    return `<section class="intro mine-view" data-testid="mine"><button class="text-link" data-action="navigate" data-route="discover">← חזרה לגילוי</button><header class="workspace-heading">${heading('האזור שלי', 'האזור שלי', 'כאן נמצאים החיפושים ששמרת, הדברים שפרסמת והבקשות והחיבורים שלך.')}</header><section class="workspace-section saved-work"><div class="section-heading"><span class="context">לעצמך</span><h2>החיפושים ששמרתי</h2></div><p class="privacy-note">החיפושים ששמרת כאן פרטיים ולא מתפרסמים.</p><div class="created-list">${intents}</div></section><section class="workspace-section published-work"><div class="section-heading"><span class="context">פעיל עכשיו</span><h2>הדברים שפרסמתי</h2></div><div class="created-list">${initiatives.length ? initiatives.map(item => itemCard(item, true)).join('') : emptyMy('עדיין אין יוזמה שפרסמת.', 'start', 'להתחיל חיפוש')}</div><div class="created-list">${published.length ? published.map(item => itemCard(item)).join('') : emptyMy('עדיין אין פרסומים נוספים.', 'start', 'להתחיל חיפוש')}</div></section><section class="workspace-section connection-work"><div class="section-heading"><span class="context">להמשיך מכאן</span><h2>הבקשות והחיבורים שלי</h2></div><div class="created-list">${connections}</div></section></section>`;
  };
  ...
  const renderManageInitiative = state => { const item = createdById(state.context.entityId); if (!item || item.type !== 'initiative') return renderMine(state); const needs = [['people', 'אנשים', item.peopleNeeded], ['place', 'מקום', item.place], ['educator', 'אשת חינוך / מקצוע', item.educator]].filter(([, , value]) => value); return `<section class="intro detail-view manage-view" data-testid="manage-initiative"><button class="text-link" data-action="navigate" data-route="mine">← חזרה לשלי</button><header class="manage-heading">${heading('ניהול יוזמה', escapeHtml(item.title), 'אפשר לעדכן כל צורך בנפרד.')}<div class="manage-status"><span>סטטוס: פורסמה</span><span>${escapeHtml(item.area)}</span></div></header><section class="manage-needs">${needs.length ? needs.map(([key, label, value]) => { const paused = item.needStates?.[key] === 'paused'; return `<article class="manage-need" data-need-key="${key}"><div><span class="context">${label}</span><p>${escapeHtml(value)} · ${paused ? 'מצאתי כרגע' : 'עדיין מחפשת'}</p></div><button class="secondary" data-action="set-initiative-need-state" data-item-id="${item.id}" data-need-key="${key}" data-need-state="${paused ? 'open' : 'paused'}">${paused ? 'לחדש' : 'להשהות'}</button></article>`; }).join('') : '<p>לא הוגדר צורך נוסף בפרסום הזה.</p>'}</section><footer class="manage-footer"><button class="secondary" data-action="open-created" data-item-id="${item.id}">לצפות בפרסום</button></footer></section>`; };
  ...
  const renderOverlay = overlay => {
    ...
    return `<div class="scrim open" data-action="close-overlay"></div><section class="sheet open start-sheet" role="dialog" aria-modal="true" data-testid="start-sheet"><button class="close" data-action="close-overlay">×</button>${heading('הצעד הראשון', 'מה היית רוצה לעשות?')}<div class="sheet-options"><button data-action="start-find"><span class="choice-number">01</span><span>אני מחפשת…<small>אנשים, מקום, עבודה, שירות או הזדמנות</small></span></button><button data-action="start-create"><span class="choice-number">02</span><span>אני רוצה להתחיל…<small>יוזמה, מפגש או מסגרת שמתאימה לחיים עם ילדים</small></span></button><button data-action="start-offer"><span class="choice-number">03</span><span>יש לי מה להציע<small>שירות, פעילות, מקום, ידע או עזרה מקצועית</small></span></button></div></section>`;
  };
